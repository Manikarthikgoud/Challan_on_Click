...
app.secret_key="************"
BASE_PATH=os.getcwd()
UPLOAD_PATH=os.path.join(BASE_PATH,'RNALPR/assets/upload')
...
def indexnative():
        ...
        text=[]
        text=(prediction(path_save,filename))
        n=len(text)
        return ({
...
def raisechallan():
        ...
        if(type(x)!=type(y)):
              ...
              msg=Message('Challan on your vehical no:'+lc_number,sender='alprchallansystem@gmail.com',recipients=[receiver_email])
...
if __name__=='__main__':
    app.run(host='192.168.112.24',debug=True)