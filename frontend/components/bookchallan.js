...
// const baseUrl="192.168.43.178"
const BookChallan=({navigation})=>{
  ...
  useEffect(()=>{
    ...
    if(token!=null){
      fetch('http://10.0.0.167:5000/protected',{
        method:"GET",
        headers:{
  ...
  const handleImageUpload=async()=>{
    ...
    if(!ImagePickerResult.canceled){
      ...
      setLoading(1)
      fetch('http://10.0.0.167:5000/upload',{
        method:'POST',
        headers:{
  ...
  const handleImageCapture=async()=>{
    ...
    if(!ImagePickerResult.canceled){
      ...
      setLoading(1)
      fetch('http://192.168.125.24:5000/upload',{
        method:'POST',
        headers:{
          'Content-Type':'multipart/form-data',
        },
